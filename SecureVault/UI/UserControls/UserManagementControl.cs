// ============================================
// SecureVault - User Management (Layout Refactored)
// ============================================
...
namespace SecureVault.UI.UserControls
{
    public class UserManagementControl : UserControl
    {
        ...
        private void BuildUI()
        {
            ...
            {
                PlaceholderText = "🔍 Search users...", Size = new Size(300, 40),
            ...
            {
                Text = "🔒 Block/Unblock", Size = new Size(160, 38),
                IsGradient = false, FlatColor = AppTheme.Warning, ForeColor = Color.Black,
                CornerRadius = 8, Margin = new Padding(0, 6, 10, 0)
            };
            ...
            {
                Text = "❌ Delete User", Size = new Size(140, 38),
                ...
                CornerRadius = 8, Margin = new Padding(0, 6, 0, 0)
            };
            ...
            _grid.Columns.Add("Status", "Status"); _grid.Columns["Status"].Width = 90; _grid.Columns["Status"].AutoSizeMode = DataGridViewAutoSizeColumnMode.None;
        ...
        private User? GetSel() => _grid.SelectedRows.Count > 0 && _grid.SelectedRows[0].Index < _users.Count ? _users[_grid.SelectedRows[0].Index] : null;

        private void ToggleBlock(object? sender, EventArgs e)
        {
            var u = GetSel(); if (u == null) return;
            if (u.IsBlocked) { _userService.UnblockUser(u.UserID); LoadUsers(); }
            else if (MessageBox.Show($"Block '{u.FullName}'?", "Confirm", MessageBoxButtons.YesNo) == DialogResult.Yes)
            { _userService.BlockUser(u.UserID); LoadUsers(); }
        }

        private void DeleteUser(object? sender, EventArgs e)
        {
            var u = GetSel(); if (u == null) return;
            if (MessageBox.Show($"Permanently delete '{u.FullName}'?\nAll their documents will also be deleted!", "Confirm",
                MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.Yes)
            { _userService.DeleteUser(u.UserID); LoadUsers(); }
        }
    }
}
